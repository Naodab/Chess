...
public class StepService {
    ...
    public StepResponse create(StepRequest request) {
        ...
        MatchWithHuman match = matchWithHumanRepository
    ...
    public StepResponse toBot(StepToBotRequest request) {
        MatchWithBot mwb = matchWithBotRepository
                .findById(request.getMatchId())
                .orElseThrow(() -> new AppException(ErrorCode.MATCH_NOT_EXISTED));
        ...
        int turnDraw =
                (bestMove.isAttack() || bestMove.getMovedPiece().getPieceType().isPawn())
                        ? 0
        ...
        if (mwb.getWinner() != null) response.setWinner(mwb.getWinner().name());
        return response;
    }
