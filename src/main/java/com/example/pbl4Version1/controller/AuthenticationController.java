...
public class AuthenticationController {
    ...
    ApiResponse<AuthenticationResponse> authenticate(@RequestBody AuthenticationRequest request, HttpSession session) {
        ...
        session.setAttribute("user", userService.getUserByUsernameOrEmail(request.getUsername()));
    ...
    @PostMapping("/logout")
    ApiResponse<?> logout(@RequestBody LogoutRequest request, HttpSession session)
            throws JOSEException, ParseException {
    ...
    @PostMapping("/refresh")
    ApiResponse<AuthenticationResponse> refresh(@RequestBody RefreshRequest request, HttpSession session)
            throws JOSEException, ParseException {
    ...
    public ApiResponse<String> forgotPassword(@RequestBody ForgotRequest request) {
        ...
        String emailContent = "<p>Mã xác thực của bạn là:</p>" + "<p style='color: red; font-size: 20px;'>"
                + token.getToken() + "</p>";

        String result = "Email xác nhận reset mật khẩu đã được gửi!";
        try {
            sendingEmailService.sendHtmlMail(token.getUser().getEmail(), "Reset mật khẩu", emailContent);
    ...
    @PostMapping("/verify")
    public ApiResponse<VerifyPasswordResponse> verifyPassword(@RequestBody VerifyPasswordTokenRequest request) {
    ...
    @PostMapping("/confirm")
    public ApiResponse<String> confirmChangePassword(@RequestBody ConfirmChangePasswordRequest request) {
