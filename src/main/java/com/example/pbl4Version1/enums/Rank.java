...
public enum Rank {
	...
	public static Rank getRank(int elo) {
		if (elo < Rank.BEGGINNER.max)
			return Rank.BEGGINNER;
		if (elo < Rank.AMATEUR.max)
			return Rank.AMATEUR;
		if (elo < Rank.INTERMEDIATE.max)
			return Rank.INTERMEDIATE;
		if (elo < Rank.ADVANCED.max)
			return Rank.ADVANCED;
		if (elo < Rank.EXPERT.max)
			return Rank.EXPERT;
		if (elo < Rank.CANDIDATE_MASTER.max)
			return Rank.CANDIDATE_MASTER;
		if (elo < Rank.FIDE_MASTER.max)
			return Rank.FIDE_MASTER;
		if (elo < Rank.GRANDMASTER.max)
