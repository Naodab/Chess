...
public class MiniMax implements MoveStrategy {
        ...
        for (final Move move : board.getCurrentPlayer().getLegalMoves()) {
            Callable<MoveEvaluation> task = () -> {
                ...
                if (transition.getMoveStatus().isDone()) {
                    int currentValue = board.getCurrentPlayer().getAlliance().isWhite()
    ...
    public int min(final Board board, final int depth) {
        ...
        for (final Move move : board.getCurrentPlayer().getLegalMoves()) {
            ...
            if (moveTransition.getMoveStatus().isDone()) {
                final int currentValues = max(moveTransition.getTransitionBoard(), depth - 1);
    ...
    public int max(final Board board, final int depth) {
        ...
        for (final Move move : board.getCurrentPlayer().getLegalMoves()) {
            final MoveTransition moveTransition = board.getCurrentPlayer().makeMove(move);
            if (moveTransition.getMoveStatus().isDone()) {
                final int currentValues = min(moveTransition.getTransitionBoard(), depth - 1);
