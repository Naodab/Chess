...
public class AlphaBetaWithMoveOrderingTest implements MoveStrategy {
    ...
    public AlphaBetaWithMoveOrderingTest(final int searchDepth) {
    ...
    public Move execute(final Board board) {
        ...
        for (final Move move : this.moveSorter.sort(currentPlayer.getLegalMoves())) {
            int finalHighestSeenValue = highestSeenValue;
            int finalLowestSeenValue = lowestSeenValue;
            Callable<MoveEvaluation> task = () -> {
                final MoveTransition moveTransition = currentPlayer.makeMove(move);
                if (moveTransition.getMoveStatus().isDone()) {
                    int value = alliance.isWhite() ?
                            min(moveTransition.getTransitionBoard(), searchDepth - 1, finalHighestSeenValue, finalLowestSeenValue) :
                            max(moveTransition.getTransitionBoard(), searchDepth - 1, finalHighestSeenValue, finalLowestSeenValue);
                    return new MoveEvaluation(move, value);
                } else {
                    return new MoveEvaluation(move, alliance.isWhite() ? Integer.MIN_VALUE : Integer.MAX_VALUE);
                }
            };
        ...
        try {
            for (Future<MoveEvaluation> future : futures) {
                MoveEvaluation moveEval = future.get();
                currentValue = moveEval.getValue();
